...
export const articleCreate = Joi.object().keys({
  title: Joi.string().label('Title').min(1).max(50).required().messages({
  ...
  description: Joi.string().label('Description').min(1).required().messages({
